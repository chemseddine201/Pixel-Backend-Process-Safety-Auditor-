...
use App\Services\CoreServices\CRUDServices\CRUDServiceTypes\StoringServices\Traits\DataArrayProcessorMethods;
...
{
    use  DataArrayProcessorMethods , RelationshipsStoringMethods , StoringServiceAbstractMethods;
     ...
     * @return Model|null
     */
    protected function createDefinitionModel(array $row): Model | null
    {
        $model = app($this->definitionModelClass);
        foreach ($this->getRowFillableValues($row) as $column => $value)
        {
            $model->{$column} = $value;
        }
